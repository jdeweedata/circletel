...
export function ProductsDashboard({ defaultStatus }: { defaultStatus?: StatFilterType }) {
  ...
  const [statFilter, setStatFilter] = useState<StatFilterType>(defaultStatus || null);
      ...
      <div className="min-h-screen bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
          <div className="flex flex-col gap-4">
            <div className="h-8 bg-slate-200/60 rounded w-64 animate-pulse mb-2" />
            <div className="h-5 bg-slate-200/60 rounded w-96 animate-pulse" />
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
            {[...Array(5)].map((_, i) => (
              <Card key={i} className="animate-pulse shadow-sm border-slate-100">
                <CardContent className="p-6">
                  <div className="h-10 w-10 bg-slate-200/60 rounded-xl mb-3" />
                  <div className="h-4 bg-slate-200/60 rounded w-3/4 mb-2" />
                  <div className="h-8 bg-slate-200/60 rounded w-1/2" />
                </CardContent>
              </Card>
            ))}
          </div>
