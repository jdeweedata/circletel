...
export function UnjaniContractAuditForm() {
  ...
  const form = useForm<UnjaniAuditFormData>({
    ...
    if (draft && Object.keys(draft.data).length > 0) {
      Object.entries(draft.data).forEach(([key, value]) => {
        if (value !== undefined && value !== null && value !== '') {
          setValue(key as keyof UnjaniAuditFormData, value as any);
        }
      });
    }
  ...
  const currentProgress = useMemo(() => {
    return calculateProgress(watchedValues as any, REQUIRED_FIELDS);
  }, [watchedValues]);
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      memoizedSaveDraft(watchedValues as any, currentProgress);
    }, 500);
