...
const outlineButtonClass = cn(
  'inline-flex items-center justify-center rounded-full border-2 bg-transparent px-8 py-3.5 text-sm font-semibold transition-colors hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2',
  brandBorder.navy,
  brandText.navy,
  brandBg.navyHover,
  brandRing.orange
    ...
    <div className="mt-8 flex flex-col gap-3">
      {selected ? (
        <div className="rounded-xl border border-ui-border bg-ui-bg p-4">
          <p className={cn('font-heading text-[11px] font-bold uppercase tracking-wide', brandText.grey600)}>
            Router added to this deal
          </p>
          <p className={cn('mt-1 font-heading text-sm font-semibold', brandText.navy)}>{selected.name}</p>
          <p className={cn('mt-1 font-body text-sm', brandText.navy)}>
          ...
          <button
            ...
            className={cn(
              'mt-2 font-body text-xs font-semibold underline-offset-2 hover:underline',
              brandText.orangeAccessible
            )}
          >
            Remove router
          </button>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[90vh] w-[95vw] max-w-xl overflow-y-auto rounded-xl">
          <DialogHeader>
            <DialogTitle className={cn('font-heading text-xl', brandText.navy)}>
              Add a 5G router
            </DialogTitle>
            <DialogDescription className={cn('font-body text-sm', brandText.grey600)}>
          ...
          {routers.length === 0 ? (
            <p className={cn('font-body text-sm', brandText.navy)}>
              No approved 5G routers are in stock right now.{' '}
              <a
                href={getWhatsAppLink(
                  `Hi ${getTenantConfig().branding.companyName}, I need a 5G router for ${dealName}`
                )}
                className={cn('font-semibold underline', brandText.orangeAccessible)}
              >
                WhatsApp us
              </a>
              .
                  ...
                  <li key={router.sku}>
                    <button
                      ...
                      className={cn(
                        ...
                        active
                          ? cn(brandBorder.orange, 'bg-orange-50')
                          : cn('border-ui-border bg-white', brandBorder.navyHover)
                      )}
                    >
                      <span className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-ui-bg">
                        {router.imageUrl ? (
                          ...
                          <PiWifiHighBold className={cn('h-7 w-7', brandText.orange)} aria-hidden="true" />
                        )}
                      </span>
                      <span className="min-w-0 flex-1">
                        <span className={cn('block font-heading text-sm font-semibold', brandText.navy)}>
                          {router.name}
                        </span>
                        <span className={cn('mt-1 block font-body text-sm', brandText.grey600)}>
                          {formatFiveGCashCpePrice(router.sellInclVat)} once-off
                        </span>
                      </span>
                      {active ? (
                        <PiCheckBold className={cn('h-5 w-5 shrink-0', brandText.orange)} aria-hidden="true" />
                      ) : null}
                    </button>
                  </li>
                );
          ...
          <DialogFooter className="gap-2 sm:justify-end">
            ...
            <button
              ...
              className={cn(
                'inline-flex items-center justify-center rounded-full px-8 py-3.5 text-sm font-semibold text-white transition-colors disabled:cursor-not-allowed disabled:opacity-50',
                brandBg.orange,
                brandBg.orangeHover
              )}
            >
