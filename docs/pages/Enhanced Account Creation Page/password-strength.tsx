 ...
 */
export function PasswordStrengthCompact({ password }: PasswordStrengthProps) {
  const [score, setScore] = useState(0);
  const [level, setLevel] = useState<'weak' | 'fair' | 'good' | 'strong'>('weak');
  ...
  useEffect(() => {
    ...
    if (/[a-z]/.test(password)) calculatedScore++;
    if (/[0-9]/.test(password)) calculatedScore++;
    if (/[^a-zA-Z0-9]/.test(password)) calculatedScore++;

    setScore(calculatedScore);

    if (calculatedScore <= 2) setLevel('weak');
    else if (calculatedScore === 3) setLevel('fair');
    else if (calculatedScore === 4) setLevel('good');
        ...
        {[1, 2, 3, 4].map((segment) => (
          <div
            ...
            className={cn('h-1 w-6 rounded-full transition-colors duration-300', {
              'bg-red-500': segment <= score && level === 'weak',
              'bg-orange-500': segment <= score && level === 'fair',
              'bg-yellow-500': segment <= score && level === 'good',
              'bg-green-500': segment <= score && level === 'strong',
              'bg-gray-200': segment > score,
            })}
          />
        ))}
      </div>
