...
-- This creates everything that's missing based on verification results:
-- - 0 enums (need to create 6)
-- - 1 table exists (coverage_leads) - need 4 more tables
-- - 1 function exists - need remaining functions
-- =============================================================================
...
-- =============================================================================
-- PART 2: FIX coverage_leads TABLE COLUMNS
-- The table exists but columns are wrong type (TEXT instead of ENUM)
-- We need to alter them to use the correct enum types
-- =============================================================================
...
AND column_name IN ('customer_type', 'lead_source');

-- If they're TEXT, we need to convert them:
-- (This assumes table is empty or has compatible values)

-- Alter customer_type column to use enum
ALTER TABLE coverage_leads
ALTER COLUMN customer_type TYPE customer_type
USING customer_type::customer_type;

-- Alter lead_source column to use enum
ALTER TABLE coverage_leads
ALTER COLUMN lead_source TYPE lead_source
USING lead_source::lead_source;

SELECT 'coverage_leads columns fixed' as status;

-- =============================================================================
-- PART 3: CREATE REMAINING TABLES (4 more needed)
-- =============================================================================
...
SELECT 'consumer_orders table created' as status;
...
SELECT 'business_quotes table created' as status;
...
SELECT 'kyc_documents table created' as status;
...
SELECT 'order_status_history table created' as status;
...
SELECT 'MIGRATION COMPLETE' as status,
       'Tables: ' || (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_name IN ('coverage_leads', 'consumer_orders', 'business_quotes', 'kyc_documents', 'order_status_history'))::TEXT || ' | ' ||
       'Enums: ' || (SELECT COUNT(*) FROM pg_type WHERE typname IN ('customer_type', 'lead_source', 'order_status', 'quote_status', 'kyc_document_type', 'kyc_verification_status'))::TEXT || ' | ' ||
       'Functions: ' || (SELECT COUNT(*) FROM information_schema.routines WHERE routine_schema = 'public' AND routine_name IN ('generate_order_number', 'generate_quote_number', 'calculate_quote_totals', 'update_updated_at_column', 'track_status_change'))::TEXT as summary;
