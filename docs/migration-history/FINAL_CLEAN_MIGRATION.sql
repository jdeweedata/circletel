...
-- This drops the broken coverage_leads table and recreates everything properly
-- =============================================================================
...
-- Drop the incorrectly created coverage_leads table
DROP TABLE IF EXISTS coverage_leads CASCADE;

SELECT 'Dropped existing coverage_leads table' as status;
...
SELECT 'All 6 enums created' as status;

-- =============================================================================
-- STEP 3: CREATE ALL 5 TABLES (CORRECTLY)
-- =============================================================================
...
SELECT 'coverage_leads created' as status;
...
CREATE INDEX idx_consumer_orders_order_number ON consumer_orders(order_number);
CREATE INDEX idx_consumer_orders_status ON consumer_orders(status);
CREATE INDEX idx_consumer_orders_email ON consumer_orders(email);
CREATE INDEX idx_consumer_orders_phone ON consumer_orders(phone);
CREATE INDEX idx_consumer_orders_created_at ON consumer_orders(created_at DESC);
...
CREATE INDEX idx_kyc_documents_consumer_order ON kyc_documents(consumer_order_id) WHERE consumer_order_id IS NOT NULL;
CREATE INDEX idx_kyc_documents_business_quote ON kyc_documents(business_quote_id) WHERE business_quote_id IS NOT NULL;
...
CREATE INDEX idx_kyc_documents_customer_email ON kyc_documents(customer_email);
...
CREATE TABLE order_status_history (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  entity_type TEXT NOT NULL CHECK (entity_type IN ('consumer_orders', 'business_quotes', 'coverage_leads')),
...
CREATE INDEX idx_order_status_history_entity ON order_status_history(entity_type, entity_id);
CREATE INDEX idx_order_status_history_status_changed_at ON order_status_history(status_changed_at DESC);
...
SELECT 'All 5 functions created' as status;
...
SELECT 'All 8 triggers created' as status;
...
-- Test functions
SELECT 'Order number test: ' || generate_order_number() as test1;
SELECT 'Quote number test: ' || generate_quote_number() as test2;

-- Test insert
INSERT INTO coverage_leads (
  first_name, last_name, email, phone, address, city, province
) VALUES (
  'Test', 'User', 'test@example.com', '+27821234567', '123 Test St', 'Johannesburg', 'Gauteng'
) RETURNING id, email, customer_type, lead_source;

-- Cleanup test
DELETE FROM coverage_leads WHERE email = 'test@example.com';

SELECT 'All tests passed! Migration complete.' as final_status;

-- Expected: Tables: 5 | Enums: 6 | Functions: 5
