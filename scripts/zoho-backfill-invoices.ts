 ...
 *
 * Syncs manual CircleTel invoices to ZOHO Billing
 * Only syncs: installation, pro_rata, equipment, adjustment
...
async function syncInvoice(invoice: Invoice, index: number, total: number) {
  ...
  if (isDryRun) {
    console.log('  🔍 DRY RUN: Would create ZOHO invoice');
  ...
  try {
    ...
    if (result.success) {
      console.log(`  ✅ Created ZOHO invoice: ${result.zoho_invoice_id}`);
    } else {
...
async function main() {
  console.log('\n🔄 ZOHO Billing Invoice Backfill');
  console.log('═'.repeat(60));
  console.log(`Mode: ${isDryRun ? '🔍 DRY RUN' : '✅ LIVE SYNC'}`);
  console.log(`Batch Size: ${batchSize}`);
  console.log(`Invoice Types: ${SYNCABLE_INVOICE_TYPES.join(', ')}`);
  console.log('═'.repeat(60));

  console.log('\n📊 Fetching manual invoices...');
  ...
  if (!invoices || invoices.length === 0) {
    console.log('✅ No manual invoices found');
    return;
  }
