...
export function convertProductsToCSV(products: Product[]): string {
  ...
  const rows = products.map(product => {
    const row = [
      ...
      String(product.pricing?.download_speed ?? ''),
      String(product.pricing?.upload_speed ?? ''),
      String(product.data_limit ?? product.metadata?.data_cap ?? ''),
      String(product.metadata?.contract_months ?? ''),
      escapeCSVField(String(product.metadata?.provider_name ?? product.metadata?.provider ?? '')),
      escapeCSVField(String(product.metadata?.technology ?? '')),
