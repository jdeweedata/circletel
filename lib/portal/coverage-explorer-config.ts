...
export function coverageExplorerConfig(mode: CoverageExplorerMode) {
  if (mode === 'admin') {
    return {
      ...
      formHelp:
        'Places a CircleTel install order after Unjani NPC confirms clinic details and coverage. If the Unjani Connect kit is in the warehouse it is booked out against the site; otherwise a stock order is raised (due in 5 business days). Fulfilment is 14–21 business days from the order date.',
