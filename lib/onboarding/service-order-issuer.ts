...
function buildPdfInput(
  ...
  return {
    ...
    serviceName,
    serviceReference,
    // Unjani MSA: monthly_price is EXCL VAT (R450). PDF adds 15% → R517.50 incl.
...
function serviceOrderEmailHtml(input: {
  ...
  return `
    ...
    <div style="background-color:#f5f5f5;padding:16px;border-radius:8px;margin:16px 0;">
      ...
      <p style="margin:8px 0;"><strong>Monthly Fee:</strong> R${input.monthlyFee.toFixed(2)} ex VAT (R${(input.monthlyFee * 1.15).toFixed(2)} incl. VAT)</p>
    </div>
