 ...
 *
 * Syncs CircleTel *manual* invoices to ZOHO Billing (installation, pro-rata,
 * equipment, adjustment). Recurring invoices are deferred to Zoho Books
 * (CircleTel is invoice SoR; Books is the accounting mirror).
 *
 * Prerequisites:
...
export async function syncInvoiceToZohoBilling(
  ...
  try {
    zohoLogger.debug('[InvoiceSync] Starting sync for invoice', { invoice_id });
    ...
    // Zoho Billing only accepts manual invoice types. Recurring (and other)
    // invoices are mirrored via Zoho Books (CircleTel SoR) — leave pending so
    // books-sync-orchestrator / retry can pick them up. Do NOT mark failed.
    if (!SYNCABLE_INVOICE_TYPES.includes(invoice.invoice_type as any)) {
      zohoLogger.debug(
        '[InvoiceSync] Invoice type deferred to Zoho Books sync',
        { invoice_type: invoice.invoice_type, invoice_id }
      );
      await supabase
        .from('customer_invoices')
        .update({
          ...
          zoho_last_sync_error:
            'Deferred to Zoho Books sync (Billing path skips recurring)',
        })
        .eq('id', invoice_id);

      return {
        success: false,
        error: `Invoice type '${invoice.invoice_type}' is synced via Zoho Books, not Zoho Billing`,
      };
    }

    // Check if already synced
    if (invoice.zoho_billing_invoice_id) {
      zohoLogger.debug('[InvoiceSync] Invoice already synced', { zoho_billing_invoice_id: invoice.zoho_billing_invoice_id });
      return {
        success: true,
    ...
    zohoLogger.debug('[InvoiceSync] Checking customer sync status...');
    if (!invoice.customer?.zoho_billing_customer_id) {
      zohoLogger.debug('[InvoiceSync] Customer not synced, syncing now...');
    ...
    zohoLogger.debug('[InvoiceSync] Prerequisites met:', {
    ...
    zohoLogger.debug('[InvoiceSync] Creating ZOHO invoice:', {
      invoice_id,
    ...
    zohoLogger.debug('[InvoiceSync] Successfully created ZOHO invoice:', {
    ...
    zohoLogger.error('[InvoiceSync] Error syncing invoice', { error: error instanceof Error ? error.message : String(error) });
