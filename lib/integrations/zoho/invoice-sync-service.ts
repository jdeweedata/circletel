...
export async function syncInvoiceToZohoBilling(
  ...
  try {
    ...
    if (!SYNCABLE_INVOICE_TYPES.includes(invoice.invoice_type as any)) {
      zohoLogger.debug('[InvoiceSync] Invoice type not syncable:', invoice.invoice_type);
      throw new Error(
    ...
    if (invoice.zoho_billing_invoice_id) {
      zohoLogger.debug('[InvoiceSync] Invoice already synced:', invoice.zoho_billing_invoice_id);
      return {
        success: true,
    ...
    zohoLogger.error('[InvoiceSync] Error syncing invoice:', error);
