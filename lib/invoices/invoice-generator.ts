...
export async function generateCustomerInvoice(
  ...
  // Look up corporate linkage if customer belongs to a corporate site
  let corporate_account_id: string | null = null;
  let corporate_site_id: string | null = null;
  ...
  if (customer?.corporate_site_id) {
    corporate_site_id = customer.corporate_site_id;
    const { data: site } = await supabase
      .from('corporate_sites')
      .select('corporate_id')
      .eq('id', corporate_site_id)
      .single();
    if (site?.corporate_id) {
      corporate_account_id = site.corporate_id;
    }
  }
  ...
  const { data: invoice, error } = await supabase
    ...
    .insert({
      ...
      ...(corporate_site_id && { corporate_site_id }),
    })
    .select()
    .single();
