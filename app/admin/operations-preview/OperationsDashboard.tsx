"use client";

import Image from "next/image";
import { useState } from "react";
import type { IconType } from "react-icons";
import {
  PiArrowsClockwiseBold,
  PiCalendarCheckBold,
  PiCaretRightBold,
  PiClockBold,
  PiCurrencyDollarBold,
  PiHeadsetBold,
  PiInfoBold,
  PiMapPinLineBold,
  PiPaperPlaneTiltBold,
  PiPlusBold,
  PiShoppingCartSimpleBold,
  PiSpinnerBold,
  PiSquaresFourBold,
  PiTicketBold,
  PiTrendUpBold,
  PiUserPlusBold,
  PiUsersThreeBold,
  PiWifiHighBold,
} from "react-icons/pi";
import { CartesianGrid, Line, LineChart, XAxis, YAxis } from "recharts";

import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";
import { Avatar, AvatarFallback } from "@/components/ui/avatar";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import {
  Card,
  CardContent,
  CardDescription,
  CardFooter,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";
import {
  ChartContainer,
  ChartTooltip,
  ChartTooltipContent,
  type ChartConfig,
} from "@/components/ui/chart";
import {
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
} from "@/components/ui/collapsible";
import {
  Sidebar,
  SidebarContent,
  SidebarFooter,
  SidebarGroup,
  SidebarGroupContent,
  SidebarGroupLabel,
  SidebarHeader,
  SidebarInset,
  SidebarMenu,
  SidebarMenuAction,
  SidebarMenuButton,
  SidebarMenuItem,
  SidebarMenuSub,
  SidebarMenuSubButton,
  SidebarMenuSubItem,
  SidebarProvider,
  SidebarRail,
  SidebarSeparator,
  SidebarTrigger,
  useSidebar,
} from "@/components/ui/sidebar";
import {
  Table,
  TableBody,
  TableCell,
  TableHead,
  TableHeader,
  TableRow,
} from "@/components/ui/table";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { ToggleGroup, ToggleGroupItem } from "@/components/ui/toggle-group";
import { hasChildren } from "@/lib/admin/feature-registry";
import type { OperationsPreviewData } from "@/lib/admin/operations-preview/types";
import { cn } from "@/lib/utils";
import { createPreviewNavigationHandlers } from "./interaction";
import { dashboardNavigation } from "./navigation";
import {
  buildFinanceRows,
  buildGrowthSeries,
  buildKpis,
  buildOperationsRows,
  formatGeneratedAt,
  formatZar,
  type DashboardKpiLabel,
  type GrowthRange,
} from "./view-model";

export interface OperationsDashboardProps {
  data: OperationsPreviewData;
  onRefresh: () => void;
  isRefreshing: boolean;
}

const sidebarVariables = {
  "--sidebar-background": "0 0% 100%",
  "--sidebar-foreground": "220 46% 20%",
  "--sidebar-primary": "28 81% 51%",
  "--sidebar-primary-foreground": "0 0% 100%",
  "--sidebar-accent": "28 78% 95%",
  "--sidebar-accent-foreground": "28 78% 38%",
  "--sidebar-border": "220 13% 91%",
  "--sidebar-ring": "28 81% 51%",
} as React.CSSProperties;

const quickActions: Array<{
  label: string;
  description: string;
  icon: IconType;
}> = [
  {
    label: "Add customer",
    description: "Create a new account",
    icon: PiUserPlusBold,
  },
  {
    label: "New order",
    description: "Capture a service order",
    icon: PiShoppingCartSimpleBold,
  },
  {
    label: "Log ticket",
    description: "Open a support case",
    icon: PiTicketBold,
  },
  {
    label: "Schedule install",
    description: "Book a field visit",
    icon: PiCalendarCheckBold,
  },
  {
    label: "Send message",
    description: "Notify a customer",
    icon: PiPaperPlaneTiltBold,
  },
  {
    label: "Run coverage",
    description: "Check a new address",
    icon: PiMapPinLineBold,
  },
];

const kpiIcons: Record<DashboardKpiLabel, IconType> = {
  "Active customers": PiUsersThreeBold,
  "Active MRR": PiCurrencyDollarBold,
  "Open tickets": PiHeadsetBold,
  "Network incidents": PiWifiHighBold,
};

const chartConfig = {
  totalCustomers: {
    label: "Total customers",
    color: "#1B2A4A",
  },
  billedCents: {
    label: "Billed revenue",
    color: "#E87A1E",
  },
} satisfies ChartConfig;

function containedNavigationMessage(label: string): string {
  return `${label} is available in the production admin and remains contained in this read-only preview.`;
}

function OperationsSidebar({
  onNavigate,
}: {
  onNavigate: (label: string) => void;
}) {
  const [expandedItems, setExpandedItems] = useState<string[]>([]);
  const { isMobile, setOpen, setOpenMobile, state } = useSidebar();

  const setItemExpanded = (name: string, expanded: boolean) => {
    setExpandedItems((current) =>
      expanded
        ? [...new Set([...current, name])]
        : current.filter((itemName) => itemName !== name),
    );
  };

  const getNavigationHandlers = (label: string) =>
    createPreviewNavigationHandlers({
      label,
      onNavigate,
      onMobileClose: isMobile ? () => setOpenMobile(false) : undefined,
    });

  return (
    <Sidebar collapsible="icon" className="border-ui-border bg-white">
      <SidebarHeader className="h-16 justify-center border-b border-ui-border px-3">
        <div className="flex items-center gap-2 overflow-hidden">
          <Image
            src="/images/circletel-logo.png"
            alt="CircleTel"
            width={190}
            height={64}
            priority
            className="h-14 w-auto object-contain group-data-[collapsible=icon]:hidden"
          />
          <Image
            src="/images/circletel-enclosed-logo.png"
            alt="CircleTel"
            width={32}
            height={32}
            className="hidden object-contain group-data-[collapsible=icon]:block"
          />
        </div>
      </SidebarHeader>

      <SidebarContent className="bg-white py-3">
        <nav aria-label="Operations navigation" className="flex flex-col gap-2">
          <SidebarGroup>
            <SidebarGroupContent>
              <SidebarMenu>
                <SidebarMenuItem>
                  <SidebarMenuButton
                    {...getNavigationHandlers("Dashboard")}
                    isActive
                    aria-current="page"
                    tooltip="Dashboard"
                    className="h-10 data-[active=true]:bg-circleTel-orange-light data-[active=true]:text-circleTel-orange-accessible"
                  >
                    <PiSquaresFourBold />
                    <span>Dashboard</span>
                  </SidebarMenuButton>
                </SidebarMenuItem>
              </SidebarMenu>
            </SidebarGroupContent>
          </SidebarGroup>

          <SidebarSeparator />

          {dashboardNavigation.map((section) => (
            <SidebarGroup key={section.label}>
              <SidebarGroupLabel className="uppercase tracking-[0.12em]">
                {section.label}
              </SidebarGroupLabel>
              <SidebarGroupContent>
                <SidebarMenu>
                  {section.items.map((item) => {
                    if (hasChildren(item)) {
                      const isExpanded = expandedItems.includes(item.name);

                      return (
                        <Collapsible
                          key={item.name}
                          open={isExpanded}
                          onOpenChange={(expanded) => {
                            if (
                              expanded &&
                              !isMobile &&
                              state === "collapsed"
                            ) {
                              setOpen(true);
                            }
                            setItemExpanded(item.name, expanded);
                          }}
                        >
                          <SidebarMenuItem className="group/collapsible">
                            <SidebarMenuButton
                              {...getNavigationHandlers(item.name)}
                              data-preview-item={item.name}
                              tooltip={item.name}
                              className="h-9 pr-8 text-ui-text-secondary hover:text-circleTel-navy"
                            >
                              <item.icon />
                              <span className="truncate">{item.name}</span>
                            </SidebarMenuButton>
                            <CollapsibleTrigger asChild>
                              <SidebarMenuAction
                                type="button"
                                aria-expanded={isExpanded}
                                aria-label={`${isExpanded ? "Collapse" : "Expand"} ${item.name}`}
                                title={`${isExpanded ? "Collapse" : "Expand"} ${item.name}`}
                              >
                                <PiCaretRightBold
                                  className={cn(
                                    "motion-safe:transition-transform motion-reduce:transition-none",
                                    isExpanded && "rotate-90",
                                  )}
                                />
                              </SidebarMenuAction>
                            </CollapsibleTrigger>
                            <CollapsibleContent>
                              <SidebarMenuSub>
                                {item.children.map((child) => (
                                  <SidebarMenuSubItem key={child.href}>
                                    <SidebarMenuSubButton asChild>
                                      <button
                                        {...getNavigationHandlers(child.name)}
                                        data-preview-item={child.name}
                                        data-preview-href={child.href}
                                        className="text-ui-text-secondary hover:text-circleTel-navy"
                                      >
                                        <child.icon />
                                        <span className="truncate">
                                          {child.name}
                                        </span>
                                      </button>
                                    </SidebarMenuSubButton>
                                  </SidebarMenuSubItem>
                                ))}
                              </SidebarMenuSub>
                            </CollapsibleContent>
                          </SidebarMenuItem>
                        </Collapsible>
                      );
                    }

                    return (
                      <SidebarMenuItem key={item.name}>
                        <SidebarMenuButton
                          {...getNavigationHandlers(item.name)}
                          data-preview-item={item.name}
                          data-preview-href={item.href}
                          tooltip={item.name}
                          className="h-9 text-ui-text-secondary hover:text-circleTel-navy"
                        >
                          <item.icon />
                          <span className="truncate">{item.name}</span>
                        </SidebarMenuButton>
                      </SidebarMenuItem>
                    );
                  })}
                </SidebarMenu>
              </SidebarGroupContent>
            </SidebarGroup>
          ))}
        </nav>
      </SidebarContent>

      <SidebarFooter className="border-t border-ui-border bg-white p-3">
        <p className="text-xs text-muted-foreground group-data-[collapsible=icon]:hidden">
          Operations Console · Read-only preview
        </p>
      </SidebarFooter>
      <SidebarRail />
    </Sidebar>
  );
}

function OperationsHeader({
  onRefresh,
  isRefreshing,
}: Pick<OperationsDashboardProps, "onRefresh" | "isRefreshing">) {
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-ui-border bg-white px-4 md:px-6">
      <div className="flex min-w-0 items-center gap-3">
        <SidebarTrigger aria-label="Toggle operations navigation" />
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold text-circleTel-navy">
            Operations overview
          </p>
          <p className="hidden truncate text-xs text-muted-foreground sm:block">
            Production aggregates for staff
          </p>
        </div>
      </div>

      <div className="flex items-center gap-2">
        <Button
          type="button"
          variant="outline"
          size="sm"
          onClick={onRefresh}
          disabled={isRefreshing}
          aria-label="Refresh production data"
        >
          {isRefreshing ? (
            <PiSpinnerBold data-icon="inline-start" className="animate-spin" />
          ) : (
            <PiArrowsClockwiseBold data-icon="inline-start" />
          )}
          {isRefreshing ? "Refreshing…" : "Refresh"}
        </Button>
        <Avatar className="size-9">
          <AvatarFallback className="bg-circleTel-orange-light text-xs font-semibold text-circleTel-orange-accessible">
            CT
          </AvatarFallback>
        </Avatar>
        <span className="hidden text-sm font-semibold text-circleTel-navy lg:inline">
          CircleTel operations
        </span>
      </div>
    </header>
  );
}

function PreviewHeading({ generatedAt }: { generatedAt: string }) {
  return (
    <section className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div className="flex flex-col gap-2">
        <div className="flex flex-wrap items-center gap-2">
          <Badge variant="secondary">Production data</Badge>
          <Badge variant="outline">Read-only preview</Badge>
        </div>
        <div className="flex flex-col gap-1">
          <h1 className="text-2xl font-semibold tracking-tight text-circleTel-navy md:text-3xl">
            CircleTel operations
          </h1>
          <p className="text-sm text-muted-foreground md:text-base">
            Production-backed operational and commercial aggregates for staff.
          </p>
        </div>
      </div>
      <div className="flex flex-col gap-3 lg:items-end">
        <div className="flex flex-col gap-1 text-sm lg:items-end">
          <span className="font-medium text-circleTel-navy">Updated</span>
          <time dateTime={generatedAt} className="text-muted-foreground">
            {formatGeneratedAt(generatedAt)} · Africa/Johannesburg
          </time>
        </div>
        <Button
          type="button"
          variant="cta"
          disabled
          aria-disabled="true"
          className="w-fit"
        >
          <PiPlusBold data-icon="inline-start" />
          Create work item
          <span className="text-xs font-normal">Preview only</span>
        </Button>
      </div>
    </section>
  );
}

function ContainedNavigationAlert({ message }: { message: string }) {
  return (
    <Alert>
      <PiInfoBold />
      <AlertTitle>Read-only navigation</AlertTitle>
      <AlertDescription>{message}</AlertDescription>
    </Alert>
  );
}

function KpiGrid({ data }: { data: OperationsPreviewData }) {
  return (
    <section
      aria-label="Key performance indicators"
      className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"
    >
      {buildKpis(data).map((item) => {
        const Icon = kpiIcons[item.label];

        return (
          <Card key={item.label} className="border-ui-border shadow-sm">
            <CardHeader className="flex-row items-start justify-between gap-4">
              <div className="flex min-w-0 flex-col gap-1">
                <CardDescription>{item.label}</CardDescription>
                <CardTitle className="text-3xl text-circleTel-navy">
                  {item.value}
                </CardTitle>
              </div>
              <div className="flex size-10 shrink-0 items-center justify-center rounded-full bg-circleTel-orange-light text-circleTel-orange-accessible">
                <Icon className="size-5" aria-hidden="true" />
              </div>
            </CardHeader>
            <CardContent>
              {item.status ? (
                <Badge variant="outline">{item.status}</Badge>
              ) : (
                <Badge variant="outline">Production aggregate</Badge>
              )}
            </CardContent>
            <CardFooter>
              <p className="text-xs text-muted-foreground">{item.detail}</p>
            </CardFooter>
          </Card>
        );
      })}
    </section>
  );
}

function DisabledWorkflowButton({
  label,
  description,
  icon: Icon,
}: {
  label: string;
  description: string;
  icon: IconType;
}) {
  return (
    <Button
      type="button"
      variant="outline"
      disabled
      aria-disabled="true"
      className="h-auto justify-start px-4 py-3"
    >
      <Icon data-icon="inline-start" />
      <span className="flex min-w-0 flex-1 flex-col items-start">
        <span className="truncate font-semibold">{label}</span>
        <span className="hidden text-xs font-normal text-muted-foreground 2xl:block">
          {description}
        </span>
        <span className="text-xs font-normal text-muted-foreground">
          Preview only
        </span>
      </span>
    </Button>
  );
}

function QuickActionsDisabled() {
  return (
    <Card className="min-w-0 border-ui-border shadow-sm">
      <CardHeader className="flex-col items-start gap-4 sm:flex-row sm:justify-between">
        <div className="flex flex-col gap-1">
          <CardTitle className="text-lg text-circleTel-navy">
            Quick actions
          </CardTitle>
          <CardDescription>
            Staff workflows are visible but unavailable in this preview.
          </CardDescription>
        </div>
        <Badge variant="secondary">Preview only</Badge>
      </CardHeader>
      <CardContent className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        {quickActions.map((action) => (
          <DisabledWorkflowButton key={action.label} {...action} />
        ))}
      </CardContent>
    </Card>
  );
}

function GrowthChart({ data }: { data: OperationsPreviewData }) {
  const [range, setRange] = useState<GrowthRange>("6m");
  const growth = buildGrowthSeries(data, range);

  return (
    <Card className="min-w-0 border-ui-border shadow-sm">
      <CardHeader className="flex-col items-start gap-4 sm:flex-row sm:justify-between">
        <div className="flex flex-col gap-1">
          <CardTitle className="text-lg text-circleTel-navy">
            Growth &amp; revenue
          </CardTitle>
          <CardDescription>
            Cumulative customers and monthly billed invoice totals.
          </CardDescription>
        </div>
        <ToggleGroup
          type="single"
          value={range}
          onValueChange={(value) => {
            if (value === "6m" || value === "12m") setRange(value);
          }}
          variant="outline"
          size="sm"
          aria-label="Reporting period"
        >
          <ToggleGroupItem value="6m" aria-label="6 months">
            6m
          </ToggleGroupItem>
          <ToggleGroupItem value="12m" aria-label="12 months">
            12m
          </ToggleGroupItem>
        </ToggleGroup>
      </CardHeader>
      <CardContent>
        <div data-growth-points={growth.length}>
          <ChartContainer
            config={chartConfig}
            className="h-[320px] w-full"
            role="img"
            aria-label={`${range} chart of total customers and billed revenue`}
          >
            <LineChart
              accessibilityLayer
              data={growth}
              margin={{ left: 4, right: 4, top: 8 }}
            >
              <CartesianGrid vertical={false} />
              <XAxis
                dataKey="label"
                tickLine={false}
                axisLine={false}
                tickMargin={10}
              />
              <YAxis
                yAxisId="customers"
                tickLine={false}
                axisLine={false}
                width={42}
                allowDecimals={false}
              />
              <YAxis
                yAxisId="revenue"
                orientation="right"
                tickLine={false}
                axisLine={false}
                width={72}
                tickFormatter={(value) => formatZar(Math.round(Number(value)))}
              />
              <ChartTooltip
                cursor={false}
                content={
                  <ChartTooltipContent
                    formatter={(value, name) =>
                      name === "billedCents"
                        ? formatZar(Number(value))
                        : new Intl.NumberFormat("en-ZA").format(Number(value))
                    }
                  />
                }
              />
              <Line
                yAxisId="customers"
                dataKey="totalCustomers"
                type="monotone"
                stroke="var(--color-totalCustomers)"
                strokeWidth={3}
                isAnimationActive={false}
                dot={{ r: 3 }}
                activeDot={{ r: 5 }}
              />
              <Line
                yAxisId="revenue"
                dataKey="billedCents"
                type="monotone"
                stroke="var(--color-billedCents)"
                strokeWidth={3}
                isAnimationActive={false}
                dot={{ r: 3 }}
                activeDot={{ r: 5 }}
              />
            </LineChart>
          </ChartContainer>
        </div>
      </CardContent>
      <CardFooter className="flex-wrap gap-4 text-xs text-muted-foreground">
        <span className="flex items-center gap-2">
          <span className="size-2 rounded-full bg-circleTel-navy" />
          Total customers
        </span>
        <span className="flex items-center gap-2">
          <span className="size-2 rounded-full bg-circleTel-orange" />
          Billed revenue
        </span>
      </CardFooter>
    </Card>
  );
}

function DataRowsTable({
  caption,
  rows,
}: {
  caption: string;
  rows: Array<{ label: string; value: string }>;
}) {
  return (
    <Table>
      <caption className="sr-only">{caption}</caption>
      <TableHeader>
        <TableRow>
          <TableHead className="h-10">Metric</TableHead>
          <TableHead className="h-10 text-right">Value</TableHead>
        </TableRow>
      </TableHeader>
      <TableBody>
        {rows.map((row) => (
          <TableRow key={row.label}>
            <TableCell className="py-3 font-medium text-circleTel-navy">
              {row.label}
            </TableCell>
            <TableCell className="py-3 text-right text-lg font-semibold">
              {row.value}
            </TableCell>
          </TableRow>
        ))}
      </TableBody>
    </Table>
  );
}

function OperationsAndFinancePanel({ data }: { data: OperationsPreviewData }) {
  return (
    <Tabs defaultValue="operations" className="min-w-0">
      <Card className="min-w-0 border-ui-border shadow-sm">
        <CardHeader className="flex-col items-start gap-4 sm:flex-row sm:justify-between">
          <div className="flex flex-col gap-1">
            <CardTitle className="text-lg text-circleTel-navy">
              Command center
            </CardTitle>
            <CardDescription>
              Current operations and invoice-cohort finance.
            </CardDescription>
          </div>
          <TabsList className="grid w-full grid-cols-2 sm:w-auto">
            <TabsTrigger value="operations">Operations</TabsTrigger>
            <TabsTrigger value="finance">Finance</TabsTrigger>
          </TabsList>
        </CardHeader>
        <CardContent className="px-0">
          <TabsContent value="operations" className="mt-0">
            <div className="flex items-center justify-between px-6 pb-3">
              <div className="flex items-center gap-2">
                <PiClockBold className="size-5 text-circleTel-orange-accessible" />
                <span className="text-sm font-semibold text-circleTel-navy">
                  Operations snapshot
                </span>
              </div>
              <Badge variant="secondary">Production aggregate</Badge>
            </div>
            <DataRowsTable
              caption="Current operations snapshot"
              rows={buildOperationsRows(data)}
            />
          </TabsContent>

          <TabsContent value="finance" className="mt-0">
            <div className="flex items-center justify-between px-6 pb-3">
              <div className="flex flex-col gap-1">
                <span className="text-sm font-semibold text-circleTel-navy">
                  Current invoice cohort
                </span>
                <span className="text-xs text-muted-foreground">
                  {data.finance.periodStart} to {data.finance.periodEnd}
                </span>
              </div>
              <Badge variant="outline">Current month</Badge>
            </div>
            <DataRowsTable
              caption="Current-month finance snapshot"
              rows={buildFinanceRows(data)}
            />
          </TabsContent>
        </CardContent>
      </Card>
    </Tabs>
  );
}

export function OperationsDashboard({
  data,
  onRefresh,
  isRefreshing,
}: OperationsDashboardProps) {
  const [containedMessage, setContainedMessage] = useState<string | null>(null);

  const showContainedFeedback = (label: string) => {
    setContainedMessage(containedNavigationMessage(label));
  };

  return (
    <SidebarProvider
      defaultOpen
      className="min-h-svh bg-ui-bg"
      style={sidebarVariables}
    >
      <OperationsSidebar onNavigate={showContainedFeedback} />
      <SidebarInset className="min-w-0 bg-ui-bg">
        <OperationsHeader onRefresh={onRefresh} isRefreshing={isRefreshing} />

        <main className="flex flex-1 flex-col gap-6 p-4 md:p-6 xl:p-8">
          <PreviewHeading generatedAt={data.generatedAt} />

          {containedMessage ? (
            <ContainedNavigationAlert message={containedMessage} />
          ) : null}

          <KpiGrid data={data} />
          <QuickActionsDisabled />

          <section
            aria-label="Business and operational analytics"
            className="grid min-w-0 items-start gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(380px,0.65fr)]"
          >
            <GrowthChart data={data} />
            <OperationsAndFinancePanel data={data} />
          </section>

          <footer className="flex flex-col gap-2 border-t border-ui-border pt-4 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
            <span>CircleTel Operations Console · Read-only preview</span>
            <span className="flex items-center gap-1">
              <PiTrendUpBold /> Updated {formatGeneratedAt(data.generatedAt)}
            </span>
          </footer>
        </main>
      </SidebarInset>
    </SidebarProvider>
  );
}
