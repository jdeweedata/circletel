/**
 * Dashboard Statement PDF API
 * GET /api/dashboard/statement/pdf
 *
 * Generates and streams a jsPDF statement PDF for the authenticated customer.
 * Customer is inferred from auth — no [id] segment.
 * Query params: period (3m|6m|12m|all), from, to
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 4. Assemble statement + generate PDF
    const { statement } = await assembleStatementData(
      serviceClient,
      customer.id,
      { period, from, to }
    );
    ...
    const accountNumber = statement.customer.accountNumber || customer.id;
    ...
    billingLogger.error('[Dashboard Statement PDF] Error', {
