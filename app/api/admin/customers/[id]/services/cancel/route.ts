/**
 * Admin Service Cancellation API
 * POST /api/admin/customers/[id]/services/cancel
 * 
 * Cancel an active or suspended service
...
export async function POST(
  ...
  try {
    ...
    const { service_id, reason, notes, cancellation_date } = body;
    ...
    const result = await ServiceManager.cancelService({
      service_id,
      admin_user_id: user.id,
      reason,
      notes,
      cancellation_date: cancellation_date ? new Date(cancellation_date) : undefined
    });
    
    if (!result.success) {
      return NextResponse.json(
        { error: result.error || 'Failed to cancel service' },
        { status: 400 }
      );
    }
