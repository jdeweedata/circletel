...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      apiLogger.error('Error fetching MTN deals', { error });
    ...
    apiLogger.error('Error in MTN deals API', { error });
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      apiLogger.error('Error creating deal', { error });
      return NextResponse.json(
    ...
    apiLogger.error('Error in MTN deals POST', { error });
    return NextResponse.json(
      { success: false, error: 'Internal server error' },
      { status: 500 }
    );
