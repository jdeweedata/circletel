...
ON public.customer_invoices (status, due_date, sms_reminder_count, sms_reminder_sent_at)
WHERE status IN ('overdue', 'unpaid', 'partial');

-- Add comment for documentation
COMMENT ON COLUMN public.customer_invoices.sms_reminder_sent_at IS 'Timestamp of last SMS reminder sent via Clickatell';
COMMENT ON COLUMN public.customer_invoices.sms_reminder_count IS 'Number of SMS reminders sent (max 3)';
COMMENT ON COLUMN public.customer_invoices.sms_reminder_error IS 'Last SMS sending error message if any';

-- Grant permissions
GRANT SELECT, UPDATE ON public.customer_invoices TO authenticated;
GRANT SELECT, UPDATE ON public.customer_invoices TO service_role;
