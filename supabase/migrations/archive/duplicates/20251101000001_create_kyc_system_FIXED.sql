-- ============================================
-- KYC and RICA System (FIXED)
-- ============================================
-- Purpose: FICA-compliant KYC verification and RICA pairing
-- Features: Didit integration, risk scoring, RICA auto-submission
-- Created: 2025-11-01
-- Fixed: Removed problematic customer_id RLS policies that reference non-existent relationships
...
CREATE INDEX IF NOT EXISTS idx_kyc_verification_result ON kyc_sessions(verification_result);
...
CREATE INDEX IF NOT EXISTS idx_rica_status ON rica_submissions(status);
...
DROP POLICY IF EXISTS kyc_customer_select ON kyc_sessions;
DROP POLICY IF EXISTS kyc_admin_all ON kyc_sessions;
DROP POLICY IF EXISTS kyc_system_insert ON kyc_sessions;
DROP POLICY IF EXISTS kyc_system_update ON kyc_sessions;
...
CREATE POLICY kyc_service_insert ON kyc_sessions
  FOR INSERT
  WITH CHECK (auth.role() = 'service_role');

-- Policy: Service role can UPDATE (for webhooks)
CREATE POLICY kyc_service_update ON kyc_sessions
  FOR UPDATE
  USING (auth.role() = 'service_role')
  WITH CHECK (auth.role() = 'service_role');

-- Policy: Service role can SELECT (for API endpoints)
CREATE POLICY kyc_service_select ON kyc_sessions
  FOR SELECT
  USING (auth.role() = 'service_role');
...
-- Drop existing policies if they exist
DROP POLICY IF EXISTS rica_customer_select ON rica_submissions;
DROP POLICY IF EXISTS rica_operations_select ON rica_submissions;
DROP POLICY IF EXISTS rica_admin_all ON rica_submissions;
DROP POLICY IF EXISTS rica_system_insert ON rica_submissions;
DROP POLICY IF EXISTS rica_system_update ON rica_submissions;
...
-- Policy: Service role can INSERT and UPDATE (for webhooks)
CREATE POLICY rica_service_insert ON rica_submissions
  FOR INSERT
  WITH CHECK (auth.role() = 'service_role');

CREATE POLICY rica_service_update ON rica_submissions
  FOR UPDATE
  USING (auth.role() = 'service_role')
  WITH CHECK (auth.role() = 'service_role');

-- Policy: Service role can SELECT
CREATE POLICY rica_service_select ON rica_submissions
  FOR SELECT
  USING (auth.role() = 'service_role');
