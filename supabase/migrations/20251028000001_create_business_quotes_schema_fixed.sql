-- Business Quotes System Schema (FIXED)
...
-- Created: 2025-10-28
-- Fixed: Corrected foreign key references to match actual schema
...
CREATE TABLE business_quotes (
  ...
  -- Workflow tracking (FIXED: admin_users.id not user_id)
  ...
  -- Audit (FIXED: admin_users.id not user_id)
...
CREATE TABLE business_quote_versions (
  ...
  -- Change tracking (FIXED: admin_users.id)
...
-- FIXED: Check admin_users.id, compare with auth.uid() via customers.auth_user_id
